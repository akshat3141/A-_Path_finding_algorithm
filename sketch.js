...
let Spot = function (_x, _y) {
  ...
  this.previous; //Indicates to previous node
  this.wall = false;
  ...
  this.addNeighbours = function (grid) {
    ...
    if (y > 0) this.neighbours.push(grid[x][y - 1]); //Top neighbour
    if (y < row - 1) this.neighbours.push(grid[x][y + 1]); //Bottom neighbour
    if (x < column - 1) this.neighbours.push(grid[x + 1][y]); //Right neighbour
    if (x > 0) this.neighbours.push(grid[x - 1][y]); //Left neighbour

    //Diagonal neighbours
    if (x > 0 && y > 0) this.neighbours.push(grid[x - 1][y - 1]); //Bottom-Left neighbour
    if (x < column - 1 && y > 0) this.neighbours.push(grid[x + 1][y - 1]); //Bottom-Right neighbour
    if (x > 0 && y < row - 1) this.neighbours.push(grid[x - 1][y + 1]); //Top-Left neighbour
    if (x < column - 1 && y < row - 1) this.neighbours.push(grid[x + 1][y + 1]); //Top-Right neighbour
  };
};
